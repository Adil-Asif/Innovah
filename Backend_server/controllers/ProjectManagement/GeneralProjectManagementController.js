...
exports.returnAllData = (req, res) => {
  ...
  console.log(userid);
  let sql = `select  * from  project_details,project_members where project_details.projectid =  project_members.projectid and project_members.memberid= ?`;

  model.query(sql, [userid],(err, result) => {
    if (err) {
...
//         totalresponse[0].todotask = retunNumberOfRequiredTasks(
//           selectingOnlyTodoAndCompleted,
//           "To Do"
//         );
//         totalresponse[0].completed = retunNumberOfRequiredTasks(
//           selectingOnlyTodoAndCompleted,
//           "Completed"
//         );
...
//         onlykeys = inventoryMan.map((tasks, index) => Object.values(tasks));
//         console.log(onlykeys, "Only keys");
//         //      console.log(onlykeys.indexOf('Added'))
//         onlykeys.forEach((elements) => {
//           if (
//             elements.indexOf("Added") != -1 ||
//             elements.indexOf("Utilised") != -1
//           ) {
...
//         totalresponse[0].unutilized = retunNumberOfRequiredItems(
//           selectingOnlyUtilizedAndUn,
//           "Added"
//         );
//         totalresponse[0].utilized = retunNumberOfRequiredItems(
//           selectingOnlyUtilizedAndUn,
//           "Utilised"
//         );
...
exports.getAllIdeasData = (req, res) => {
  ...
  console.log(userid);
  try {
    let sql = `SELECT ideaid,title FROM idea`;
    console.log("Test");
    model.query(sql, (err, result) => {
      if (err) {
        console.log("###########");
