...
let response = await fetch(`https://innovah-backend.herokuapp.com/requests/getallyoursubmissions/${userId}/${props.requestID.requestid}`)
setMySubmissions(await response.json())  
  ...
  const info = async(requestedby,submittedby,requestid,postid) => {
    ...
    let response = await fetch(
      `https://innovah-backend.herokuapp.com/requests/sendinghiringmail`,
      {
        // Adding method type
        method: "POST",
